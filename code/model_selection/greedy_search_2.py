...
Slight variant where we keep trying each entity type until none of them improve.
...
class GreedySearch2:
    ...
    def search(self,search_metric,burn_in,thinning):
        ...
        improvement = True
        while improvement:
            # Do not improve an entity type that is at the end of its range of K values
            entity_types_left = [entity for entity in self.entity_types if self.current_values_K[entity] != self.ranges_K[entity][-1]]
            improvement = False
            for current_entity_type in entity_types_left:
