...
- priors = { 'alpha' = alpha_R, 'beta' = beta_R, 'lambdaF' = lambdaF, 'lambdaS' = lambdaS, 'lambdaG' = lambda },
    a dictionary defining the priors over tau, F, S, G.
    
Initialisation can be done by running the initialise() function, with argument init_S for S:
- init_S='random'   -> draw initial values randomly from priors Exp
- init_S='exp'      -> use the expectation of the priors Exp
And argument init_FG for F and G:
- init_FG='random'  -> draw initial values randomly from priors Exp
- init_FG='exp'     -> use the expectation of the priors Exp
- init_FG='kmeans'  -> use K-means on the rows for F (+0.2), and columns for G (+0.2)
The tau parameters get updated using the model's updates.
Alternatively, you can define your own initial values for F, S, G, and tau.

Usage of class:
    BNMTF = bnmtf_gibbs(R,M,K,L,priors)
    BNMTF.initisalise(init_S,init_FG)
    BNMTF.run(iterations)
Or:
    BNMTF = bnmtf_gibbs(R,M,K,L,priors)
    BNMTF.train(init_S,init_FG,iterations)
...
from DI_MMTF.code.distributions.gamma import gamma_draw
from DI_MMTF.code.distributions.truncated_normal import TN_draw
from DI_MMTF.code.distributions.truncated_normal_vector import TN_vector_draw
...
class bnmtf_gibbs:
    def __init__(self,R,M,K,L,priors):
        ...
        self.alpha, self.beta, self.lambdaF, self.lambdaS, self.lambdaG = \
            float(priors['alpha']), float(priors['beta']), float(priors['lambdaF']), float(priors['lambdaS']), float(priors['lambdaG'])
    ...
    def train(self,init_S,init_FG,iterations):
        self.initialise(init_S=init_S,init_FG=init_FG)
        return self.run(iterations)


    # Initialise F, S, G, and tau. If init='random', draw values from an Exp and Gamma distribution. If init='exp', set it to the expectation values.
    def initialise(self,init_S='random',init_FG='random'):
        ...
        assert init_FG in ['random','exp','kmeans'], "Unknown initialisation option for F, G: %s. Should be 'random', 'exp', or 'kmeans." % init_FG
                
        self.S = 1./self.lambdaS * numpy.ones((self.K,self.L))
        if init_S == 'random':
            for k,l in itertools.product(xrange(0,self.K),xrange(0,self.L)):  
                self.S[k,l] = exponential_draw(self.lambdaS)
                
        self.F, self.G = 1./self.lambdaF * numpy.ones((self.I,self.K)), 1./self.lambdaG * numpy.ones((self.J,self.L))
        if init_FG == 'random':
            for i,k in itertools.product(xrange(0,self.I),xrange(0,self.K)):        
                self.F[i,k] = exponential_draw(self.lambdaF)
            for j,l in itertools.product(xrange(0,self.J),xrange(0,self.L)):
                self.G[j,l] = exponential_draw(self.lambdaG)
        elif init_FG == 'kmeans':
    ...
    def muF(self,tauFk,k):
        return 1./tauFk * (-self.lambdaF + self.tau*(self.M * ( (self.R-self.triple_dot(self.F,self.S,self.G.T)+numpy.outer(self.F[:,k],numpy.dot(self.S[k],self.G.T)))*numpy.dot(self.S[k],self.G.T) )).sum(axis=1)) 
        
    def tauS(self,k,l):       
        return self.tau * ( self.M * numpy.outer(self.F[:,k]**2,self.G[:,l]**2) ).sum()
        
    def muS(self,tauSkl,k,l):
        return 1./tauSkl * (-self.lambdaS + self.tau*(self.M * ( (self.R-self.triple_dot(self.F,self.S,self.G.T)+self.S[k,l]*numpy.outer(self.F[:,k],self.G[:,l]))*numpy.outer(self.F[:,k],self.G[:,l]) )).sum()) 
        
    def tauG(self,l):       
        return self.tau * ( self.M.T * numpy.dot(self.F,self.S[:,l])**2 ).T.sum(axis=0)
        
    def muG(self,tauGl,l):
        return 1./tauGl * (-self.lambdaG + self.tau*(self.M * ( (self.R-self.triple_dot(self.F,self.S,self.G.T)+numpy.outer(numpy.dot(self.F,self.S[:,l]),self.G[:,l])).T * numpy.dot(self.F,self.S[:,l]) ).T).sum(axis=0)) 
        

    # Return the average value for F, S, G, tau - i.e. our approximation to the expectations. 
    ...
    def predict_while_running(self):
        ...
        return self.compute_statistics(self.M,self.R,R_pred)
