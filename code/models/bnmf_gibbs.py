...
class bnmf_gibbs:
    def __init__(self,R,M,K,priors):
        ...
        self.alpha, self.beta, self.lambdaU, self.lambdaV = \
            float(priors['alpha']), float(priors['beta']), float(priors['lambdaU']), float(priors['lambdaV'])
    ...
    def initialise(self,init='random'):
        ...
        self.U, self.V = 1./self.lambdaU * numpy.ones((self.I,self.K)), 1.0/self.lambdaV * numpy.ones((self.J,self.K))
        if init == 'random':
            for i,k in itertools.product(xrange(0,self.I),xrange(0,self.K)):
                self.U[i,k] = exponential_draw(self.lambdaU)
            for j,k in itertools.product(xrange(0,self.J),xrange(0,self.K)):
                self.V[j,k] = exponential_draw(self.lambdaV)
    ...
    def muU(self,tauUk,k):
        return 1./tauUk * (-self.lambdaU + self.tau*(self.M * ( (self.R-numpy.dot(self.U,self.V.T)+numpy.outer(self.U[:,k],self.V[:,k]))*self.V[:,k] )).sum(axis=1)) 
        
    def tauV(self,k):
        return self.tau*(self.M.T*self.U[:,k]**2).T.sum(axis=0)
        
    def muV(self,tauVk,k):
        return 1./tauVk * (-self.lambdaV + self.tau*(self.M.T * ( (self.R-numpy.dot(self.U,self.V.T)+numpy.outer(self.U[:,k],self.V[:,k])).T*self.U[:,k] )).T.sum(axis=0)) 
