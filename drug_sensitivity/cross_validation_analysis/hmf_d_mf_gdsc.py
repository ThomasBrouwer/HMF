...
file_performance = 'results/hmf_d_mf.txt'
R, C = [], []
l = 0
...
for i,(train,test) in enumerate(zip(folds_training,folds_test)):
    ...
    ''' Predict values. '''
    D = [(R_gdsc,    train,     'Cell_lines', alpha[0]), 
         (R_ctrp,    M_ctrp,    'Cell_lines', alpha[1]), 
         (R_ccle_ic, M_ccle_ic, 'Cell_lines', alpha[2]),
         (R_ccle_ec, M_ccle_ec, 'Cell_lines', alpha[3])]
    ...
    R_pred = HMF.return_Dl(l=l,burn_in=burn_in,thinning=thinning)
