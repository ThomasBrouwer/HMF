...
where we group the columns by the number of observed entries we have in M.
"""
...
entries_per_column = M.sum(axis=0)
buckets = [200, 372, 380, 384, 386] # Bucket thresholds; first is 134-200 inclusive
column_indices_per_bucket = [set([]) for bucket_max in buckets]
indices_per_bucket = [[] for bucket_max in buckets]
for i,j in indices:
    for b,bucket_max in enumerate(buckets):
        if entries_per_column[j] <= bucket_max:
            column_indices_per_bucket[b].add(j), indices_per_bucket[b].append((i,j))
            break
print "Columns per bucket: %s." % ([len(indices_list) for indices_list in column_indices_per_bucket])
...
real_pred_per_bucket_per_method = [
    [
        [
            (real, pred) for (i,j,real,pred) in i_j_real_pred if j in column_indices_per_bucket[b]
        ]
        for b,indices_bucket_b in enumerate(column_indices_per_bucket)
    ]
...
folder_plots = "./"
plot_file = folder_plots+"performance_grouped_columns.png"


''' Plot the performances in a bar chart. '''
fig = plt.figure(figsize=(4,2))
fig.subplots_adjust(left=0.11, right=0.99, bottom=0.165, top=0.98)
plt.xlabel("Number of observed entries per column", fontsize=9, labelpad=1)
