...
settings = {
    'priorF'  : 'exponential',
    'priorG'  : ['normal','normal','normal'], #GE,ME
    'orderF'  : 'columns',
    'orderG'  : ['rows','rows','rows'],
    'ARD'     : True
}
hyperparameters = {
    'alphatau' : 1.,
...
all_K_alpha = [ # alpha order: GE, GM, PM
...
def run_all_settings(all_K_alpha):
    fout = open('results_mf_ge_gm_to_pm_std_varying_importance.txt','w')
    ...
    for K, alpha in all_K_alpha:
        ...
        for i, (train_index, test_index) in enumerate(folds):
            ...
            performances = HMF.predict_Dl(l=2,M_pred=M_Y_test,burn_in=burn_in,thinning=thinning)
